/**
 * 労働組合員フラグの自動クリア — derivation/unionFields.ts
 *
  ...
  if ('positionBand' in effectiveChanges) {
    ...
    if (posEntry) {
      ...
      if (onlyNonMember
          ...
          && resolveFieldStrictness('positionUnionFlag', strictnessOverrides ?? {}) !== 'free') {
  ...
  if ('band' in effectiveChanges) {
    ...
    if (bandEntry) {
      ...
      if (onlyNonMember
          ...
          && resolveFieldStrictness('unionFlag', strictnessOverrides ?? {}) !== 'free') {
