...
/** 社員（isRegularEmployee = true）かどうか — 循環依存回避のため derive 内でローカル定義 */
...
export function deriveFieldUpdates(
  ...
  if ('positionBand' in changes && !('band' in changes) && isRegularEmp(draft, masters)) {
    ...
    result.band = changes.positionBand
  }
  ...
  // positionBand / band → 労働組合員フラグの自動クリア
  Object.assign(result, deriveUnionFlags(draft, effectiveChanges, masters))
