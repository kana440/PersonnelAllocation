...
export function ConcurrentSecondmentInWidget({ rowId, sfIntegrated, label, isActive, onConfirm, onCancel }: Props) {
  ...
  const handleConfirm = () => {
    ...
    if (concurrentReason)             inputs.concurrentReason             = concurrentReason
    onConfirm(inputs)
  }

  return (
    <div className="mt-2 border border-indigo-200 rounded-xl overflow-hidden">
      <div className="px-3 pt-2 pb-1.5 bg-indigo-50 border-b border-indigo-100">
        <span className="text-xs font-semibold text-indigo-700">{label ?? '兼務出向受入の確認'}</span>
        <span className="ml-2 text-xs text-indigo-600">{name}</span>
        <span className="ml-1 text-xs text-indigo-400">{sfIntegrated ? '（SF統合）' : '（SF非統合）'}</span>
      </div>
      ...
      <div className="px-3 py-3 space-y-3">
        ...
        <div>
          <label className="text-xs font-medium text-gray-700 block mb-0.5">
            出向先組織コード <span className="text-red-400">*</span>
          </label>
        ...
        <div>
          ...
          {employmentTypeOptions.length > 0 ? (
            <select value={employmentType}
              ...
              className={`w-full text-xs border rounded px-2 py-1.5 bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400 ${
        ...
        {/* 兼務理由（任意） */}
        <div>
          <label className="text-xs font-medium text-gray-700 block mb-0.5">兼務理由（任意）</label>
          <input type="text" value={concurrentReason}
            onChange={e => setConcurrentReason(e.target.value)}
            placeholder="兼務理由を入力"
            className="w-full text-xs border border-gray-300 rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-indigo-400"
          />
        </div>
      </div>
      ...
      {isActive && (
        <div className="bg-indigo-50 px-3 py-2.5 flex gap-2 border-t border-indigo-100">
          <button onClick={handleConfirm} disabled={!canSubmit}
            className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-lg transition-colors">
            確認して適用
          </button>
