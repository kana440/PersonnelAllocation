...
// AllocationRow は zod スキーマを持たないため、行保存は緩い形（rowId必須のみ）で受ける。
const allocationRowsSchema = z.array(z.object({ rowId: z.number() }).passthrough())
const submitBodySchema     = z.object({ force: z.boolean().optional() })
const requestRevisionSchema = z.object({ comment: z.string().min(1) })
...
const app = new Hono<{ Variables: AuthVariables }>()
  ...
  // ── 依頼作成 ───────────────────────────────────────────────────────────
  .post('/', requireRole('admin', 'coordinator'), zValidator('json', createSubmissionSchema), async (c) => {
    const user = c.get('user')
  ...
  // ── 行保存 ─────────────────────────────────────────────────────────────
  .put('/:id/rows', zValidator('json', allocationRowsSchema), async (c) => {
    const user = c.get('user')
    const db = await getDb()
    const [sub] = await db
      .select()
    ...
    const rows     = c.req.valid('json') as AllocationRow[]
    const allRows  = await loadRoundRows(db, sub.roundCompanyId)
    const scope    = JSON.parse(sub.scope) as SubmissionScope
    const scopeIds = new Set(resolveScope(scope, allRows))
  ...
  // ── 提出 ───────────────────────────────────────────────────────────────
  .post('/:id/submit', zValidator('json', submitBodySchema.optional()), async (c) => {
    const user = c.get('user')
    const db = await getDb()
    const [sub] = await db
      .select()
    ...
    const body  = c.req.valid('json') ?? { force: false }
  ...
  // ── 手動マージ ─────────────────────────────────────────────────────────
  .post('/:id/merge', requireRole('admin', 'coordinator'), async (c) => {
    const user = c.get('user')
    const db = await getDb()
    const [sub] = await db
      .select()
  ...
  // ── 途中取り込み（sync）────────────────────────────────────────────────
  .post('/:id/sync', requireRole('admin', 'coordinator'), async (c) => {
    const user = c.get('user')
    const db = await getDb()
    const [sub] = await db
      .select()
  ...
  // ── 差し戻し ───────────────────────────────────────────────────────────
  .post('/:id/request-revision', requireRole('admin', 'coordinator'), zValidator('json', requestRevisionSchema), async (c) => {
    const user = c.get('user')
