...
const app = new Hono<{ Variables: AuthVariables }>()
  ...
  // ── Round 一覧 ─────────────────────────────────────────────────────────
  .get('/', requireRole('admin', 'coordinator'), async (c) => {
  ...
  .post('/', requireRole('admin'), zValidator('json', createRoundSchema), async (c) => {
  ...
  // ── Round 内の Company 一覧 ────────────────────────────────────────────
  .get('/:id/companies', requireRole('admin', 'coordinator'), async (c) => {
    const db = await getDb()
    const rcs = await db
      .select({
  ...
  .get('/:id/companies/:companyId/masters', requireRole('admin', 'coordinator', 'member'), async (c) => {
    const db = await getDb()
    const [rc] = await db
      .select({ id: roundCompanies.id })
      .from(roundCompanies)
  ...
  .get('/:id/companies/:companyId/excel', requireRole('admin', 'coordinator', 'member'), async (c) => {
    const db = await getDb()
    const [rc] = await db
      .select({ id: roundCompanies.id })
      .from(roundCompanies)
  ...
  .post('/:id/finalize', requireRole('admin'), async (c) => {
    const db = await getDb()
    const [round] = await db
  ...
  // ── 提出状況ダッシュボード（委任ツリー）────────────────────────────────
  .get('/:id/tree', requireRole('admin'), async (c) => {
    const db = await getDb()
    const [round] = await db
      .select({ id: rounds.id })
      .from(rounds).where(eq(rounds.id, c.req.param('id'))).limit(1)
