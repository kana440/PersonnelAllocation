...
**確認ポイント**:
- findPersons を先に呼んでいないか（propose_* でなく read ツールなので OK だが、効率を確認）
- `getChangedRows` の `name` フィルタだけで返答できているか
...
**確認ポイント**:
- `findPersons` を先に呼んでいないか（getValidationIssues の name フィルタで1ターン完結するはず）
- name フィルタが部分一致で動作しているか
...
**確認ポイント**:
- findPersons を先に呼んでいないか（propose_transfer の name フィルタで直接渡すはず）
...
- findPersons を経由していないか（1ターンで confirm まで到達するはず）

---

## T-03: rowIds 明示指定での異動（findPersons 後）

**前提**: ユーザーがすでに findPersons で山田さんの rowId=123 を取得している

**入力**: `rowId 123 の行を営業二課に移して`
...
**確認ポイント**:
- ピン留め情報や会話コンテキストから rowId を直接使えるか
- findOrgs で targetOrgCode を取得するか（orgCode が不明な場合）
...
**期待ツール呼び出し**:
```
propose_transfer({ name: "田中一郎", targetOrgCode: "..." })
→ エラー: "対象者が見つかりません"
```

**期待回答**:
- 「田中一郎さんが見つかりませんでした。氏名を確認してください」
- findPersons で候補を探す提案をしてもよい
...
**期待ツール呼び出し**:
```
findOrgs({ name: "第5営業部" })   ← 結果なし
```

**期待回答**:
- 「第5営業部という組織が見つかりませんでした」
- 類似名称の組織があれば候補を提示する
...
**確認ポイント**:
- システムプロンプトの「操作完了後は getValidationDiagnosis を呼ぶ」が守られているか
