...
export const VALUE_RULES: ValueRule[] = [
  ...
  { kind: 'constraint', field: 'payGrade',
    ...
    source:  (cl, row) => {
      ...
      const compensationCat = row?.jobType
        ? cl.subJobFamilies.find(e => e.label === (row.jobType as string))?.compensationCategory
        : undefined
      return cl.payGrades.filter(e => {
        if (!e.isEmployee) return false
  ...
  { kind: 'constraint', field: 'positionUnionFlag',
    ...
    source:  (cl, row) => {
      ...
      return (pos && !pos.isEmployeeOrAcceptedUnionMember)
  ...
  { kind: 'constraint', field: 'unionFlag',
    ...
    source:  (cl, row) => {
      ...
      return (band && !band.isEmployeeOrAcceptedUnionMember)
        ? [UNION_MEMBER_CODE.NON_MEMBER] : [...UNION_MEMBER_CODES]
    },
    message: _ => '労働組合員は有効な選択肢から選択してください' },
  ...
  // jobFamily: 裁量対象 subJobFamily を持つ親のみ
  // jobType: isDiscretionaryTarget=true の subJobFamily のみ（jobFamily 選択中はさらに絞る）
  { kind: 'constraint', field: 'positionBand',
  ...
  { kind: 'constraint', field: 'jobFamily',
    when:    row => row.positionDiscretionaryWorkFlag === DISCRETIONARY_YES,
    source:  cl => {
      const targetCodes = new Set(cl.subJobFamilies.filter(e => e.isDiscretionaryTarget).map(e => e.jobFamilyCode))
  ...
  { kind: 'constraint', field: 'jobType',
    when:    row => row.positionDiscretionaryWorkFlag === DISCRETIONARY_YES,
    source:  (cl, row) => {
      const candidates = cl.subJobFamilies.filter(e => e.isDiscretionaryTarget)
  ...
  { kind: 'constraint', field: 'positionDiscretionaryWorkFlag',
    ...
    source:  (cl, row) => {
      ...
      const subFamily = cl.subJobFamilies.find(e => e.label === (row.jobType as string | undefined))
